...
function ProductPage() {
  ...
  return <>
    ...
    <Sec style={{ background: C.white, paddingTop: 64, paddingBottom: 64 }}><Reveal><div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 48 }}>{[{ ic: "shield", t: "HIPAA Compliant", s: "End-to-end encryption" }, { ic: "lock", t: "SOC 2 Type II", s: "Audited security controls" }, { ic: "medical", t: "BAA Ready", s: "Business associate agreements" }].map((b, i) => <React.Fragment key={i}>{i > 0 && <div style={{ width: 1, height: 36, background: C.border }} />}<div style={{ display: "flex", alignItems: "center", gap: 14 }}><div style={{ width: 48, height: 48, borderRadius: "50%", border: `1.5px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center", background: C.bg }}><Icon name={b.ic} size={22} color={C.textSoft} /></div><div><div style={{ fontFamily: F.sans, fontSize: 13, fontWeight: 600, color: C.text }}>{b.t}</div><div style={{ fontFamily: F.sans, fontSize: 11, color: C.textFaint, fontWeight: 350 }}>{b.s}</div></div></div></React.Fragment>)}</div></Reveal></Sec>
    <CtaBlock />
  </>;
}
