...
.nav-inner{padding:14px 20px !important;gap:12px}
.nav-inner>div:first-child span{font-size:20px !important}
.nav-links{gap:12px !important}
.nav-links>:not(:last-child){display:none !important}
.stats-grid{grid-template-columns:1fr !important;gap:14px !important}
.solutions-grid{grid-template-columns:1fr 1fr !important}
.stat-num{font-size:clamp(48px,14vw,82px) !important}
.two-col{grid-template-columns:1fr !important;gap:32px !important}
.dash-mock{display:none !important}
...
.comparison-row>div{padding:12px 20px !important}
}
`;
...
const Sec = ({ children, style: st = {}, dark, id }) => <section id={id} className="sec" style={{ padding: "140px 48px", background: dark ? C.dark : "transparent", position: "relative", overflow: "hidden", ...st }}><div style={{ maxWidth: 1100, margin: "0 auto", position: "relative", zIndex: 1 }}>{children}</div></section>;
...
/* Vaklab Logo — stethoscope + voice waves */
const Logo = ({ size = 17 }) => <svg width={size} height={size} viewBox="0 0 48 48" fill="none">
  <circle cx="16" cy="8" r="2.5" stroke="white" strokeWidth="1.8" fill="none" />
  <circle cx="32" cy="8" r="2.5" stroke="white" strokeWidth="1.8" fill="none" />
  <path d="M16 10.5L24 22" stroke="white" strokeWidth="1.8" strokeLinecap="round" />
  <path d="M32 10.5L24 22" stroke="white" strokeWidth="1.8" strokeLinecap="round" />
  <circle cx="24" cy="27" r="4.5" stroke="white" strokeWidth="1.8" fill="none" />
  <circle cx="24" cy="27" r="1.8" fill="white" />
  <path d="M18 33C18 33 20.5 37 24 37C27.5 37 30 33 30 33" stroke="white" strokeWidth="1.6" strokeLinecap="round" fill="none" />
  <path d="M14 37C14 37 18 42 24 42C30 42 34 37 34 37" stroke="white" strokeWidth="1.5" strokeLinecap="round" fill="none" />
  <path d="M10 41C10 41 15.5 47 24 47C32.5 47 38 41 38 41" stroke="white" strokeWidth="1.4" strokeLinecap="round" fill="none" />
</svg>;
...
function Nav() {
  ...
  return (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 200, background: sc ? "rgba(248,247,244,0.82)" : "transparent", backdropFilter: sc ? "blur(28px) saturate(1.5)" : "none", borderBottom: sc ? `1px solid rgba(226,224,218,0.45)` : "none", transition: "all 0.6s cubic-bezier(0.4,0,0.2,1)" }}>
      <div className="nav-inner" style={{ maxWidth: 1180, margin: "0 auto", padding: "18px 48px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div className="nav-links" style={{ display: "flex", gap: 36, alignItems: "center" }}>
          ...
          <Btn v="ghost" onClick={scrollToDemo} style={{ fontSize: 12.5 }}>Try Sophie</Btn>
          <Btn style={{ padding: "11px 26px" }} onClick={openCalendly}>Book a Call</Btn>
        </div>
      </div>
    </nav>
  );
...
function HomeHero() {
  ...
  return <Sec style={{ paddingTop: 170, paddingBottom: 80, minHeight: "100vh", display: "flex", alignItems: "center" }}>
    ...
    <div style={{ textAlign: "center", maxWidth: 900, margin: "0 auto" }}>
      <div style={a(0)}><Label>Voice & SMS AI Agents for Healthcare</Label></div>
      ...
      <div style={{ ...a(0.18), maxWidth: 500, margin: "40px auto 0" }}><Txt style={{ fontSize: 18 }}>Book appointments, collect payments, send intake forms, and answer patient calls - 24/7</Txt></div>
      <div style={{ ...a(0.32), marginTop: 72, display: "inline-flex", flexDirection: "column", alignItems: "center", gap: 22 }}>
        <Label>MEET SOPHIE</Label>
...
const CALENDLY_URL = "https://calendly.com/pscheuermann94-vaklabai/30min";
...
function SmsDemo() {
  ...
  return (
    <Glass style={{ padding: "32px 36px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", maxWidth: 480, margin: "0 auto", width: "100%" }}>
      <Heading s="sm" style={{ marginBottom: 10 }}>Prefer to text?</Heading><Txt>Sophie does that, too.</Txt>
      ...
      {status === "sent" && <Txt style={{ fontSize: 12.5, color: C.tealMid, marginTop: 14, fontWeight: 500 }}>Check your phone — Sophie will text you!</Txt>}
...
function HomeLiveDemo() {
    ...
    ws.onmessage = (e) => {
      if (typeof e.data === "string") {
        try {
          ...
          if (data.transcript) {
            const speaker = data.role === "user" ? "Guest" : "Sophie";
  ...
  return <Sec id="live-demo" style={{ background: C.white }}>
    <Reveal><div style={{ textAlign: "center", marginBottom: 56 }}><Label>Hearing is believing</Label><Heading s="lg">Experience a live call<br />with Sophie, our AI agent</Heading></div></Reveal>
    ...
    <Reveal delay={0.14}><div style={{ display: "flex", flexDirection: "column", gap: 28, maxWidth: 760, margin: "0 auto" }}>
      <Glass hover={false} style={{ padding: "48px 40px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center" }}>
        ...
        {transcript.length > 0 && <div ref={transcriptRef} style={{ fontFamily: F.sans, fontSize: 14, color: C.textMid, lineHeight: 1.8, marginBottom: 20, minHeight: 120, maxHeight: 280, overflow: "auto", textAlign: "left", width: "100%", padding: "16px 20px", background: C.bg, borderRadius: 14, border: `1px solid ${C.borderLight}` }}>{transcript.map((t, i) => <div key={i} style={{ marginBottom: 10 }}><span style={{ fontFamily: F.mono, fontSize: 11, fontWeight: 600, color: t.speaker === "Sophie" ? C.accentMid : C.tealMid, marginRight: 10 }}>{t.speaker}</span><span>{t.text}{!t.final && <span style={{ display: "inline-block", width: 6, height: 14, background: C.accentMid, marginLeft: 2, animation: "pulse 1s ease-in-out infinite", verticalAlign: "text-bottom" }} />}</span></div>)}</div>}
...
function HomeValueProps() {
  ...
  return <Sec>
    ...
    <div className="stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>{cards.map((c, i) => <Reveal key={i} delay={i * 0.08}><Glass style={{ padding: "48px 32px", background: c.grad, height: "100%" }}>
      ...
      <div className="stat-num" style={{ fontFamily: F.serif, fontSize: 82, color: C.text, letterSpacing: "-0.05em", lineHeight: 0.88, marginBottom: 18, fontWeight: 400 }}>{c.stat}</div>
      <Txt style={{ fontSize: 14.5 }}>{c.desc}</Txt>
    </Glass></Reveal>)}</div>
    <Reveal delay={0.3}><div style={{ textAlign: "center", marginTop: 48 }}><Btn v="ghost" onClick={openCalendly}>Book a Call →</Btn></div></Reveal>
  </Sec>;
}
...
function DashboardShowcase() {
  ...
  return <div className="dash-mock"><Sec style={{ background: C.white }}>
...
function ProductPage() {
  ...
  return <>
    <Sec style={{ paddingTop: 160, minHeight: "88vh" }}><Orb color={C.accentSoft} size={600} top={-200} left={-200} blur={80} /><Blob color={C.warmSoft} size={250} top={200} right={0} opacity={0.05} /><div className="two-col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 72, alignItems: "center" }}><div><div style={a(0.06)}><Heading s="hero" style={{ fontSize: "clamp(44px,5.5vw,74px)" }}>Meet Sophie.<br /><span style={{ fontStyle: "italic", color: C.accentMid }}>The AI assistant</span><br />purpose-built for<br />healthcare.</Heading></div><div style={{ ...a(0.2), marginTop: 36 }}><Btn v="accent" style={{ padding: "16px 38px", fontSize: 15 }}>Try it Out</Btn></div></div>
    ...
    <Sec style={{ background: C.white }}><Reveal><div style={{ textAlign: "center", marginBottom: 72 }}><Label>Features</Label><Heading s="lg">Vaklab AI offers:</Heading></div></Reveal><div className="feature-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>{features.map((f, i) => <Reveal key={i} delay={i * 0.05}><Glass style={{ padding: "48px 40px", height: "100%" }}><div style={{ display: "block", marginBottom: 16 }}><Icon name={f.icon} size={28} color={C.accentMid} /></div><Heading s="sm" style={{ marginBottom: 8 }}>{f.title}</Heading><Txt style={{ fontSize: 14.5 }}>{f.desc}</Txt></Glass></Reveal>)}</div></Sec>
    <Sec><Reveal><div style={{ textAlign: "center", marginBottom: 64 }}><Label>Get results</Label><Heading s="lg">Vaklab AI delivers:</Heading></div></Reveal><div className="stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>{[{ c: `linear-gradient(160deg,${C.warmSoft},${C.bg})`, t: "More revenue", s: "without more headcount" }, { c: `linear-gradient(160deg,${C.accentSoft},${C.bg})`, t: "Patient-first timelines", s: "not staff timelines" }, { c: `linear-gradient(160deg,${C.tealSoft},${C.bg})`, t: "Exemplary care", s: "for every patient" }].map((item, i) => <Reveal key={i} delay={i * 0.06}><Glass style={{ padding: "52px 28px", textAlign: "center", background: item.c, height: "100%" }}><Heading s="sm" style={{ marginBottom: 8 }}>{item.t}</Heading><Txt style={{ fontSize: 13.5 }}>{item.s}</Txt></Glass></Reveal>)}</div></Sec>
    <Sec style={{ background: C.white }}><Reveal><div className="two-col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 72, alignItems: "center" }}><div><Label>Use Cases</Label><Heading s="lg">Sophie handles:</Heading><div style={{ marginTop: 28 }}><Btn v="accent">Try it Out</Btn></div></div><div>{cases.map((c, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 16, padding: "20px 0", borderBottom: `1px solid ${C.borderLight}`, transition: "padding-left 0.3s" }} onMouseEnter={e => e.currentTarget.style.paddingLeft = "8px"} onMouseLeave={e => e.currentTarget.style.paddingLeft = "0"}><div style={{ width: 36, height: 36, borderRadius: 10, background: C.warmSoft, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Icon name={["calendar", "pill", "card", "refresh", "clipboard", "phone"][i]} size={18} color={C.warmMid} /></div><span style={{ fontFamily: F.serif, fontSize: 20, color: C.text, fontWeight: 400 }}>{c}</span></div>)}</div></div></Reveal></Sec>
...
function Footer() {
  ...
  const cols = [
    { title: "Product", items: [{ l: "Meet the Product", p: "/product" }, { l: "Book a Call", href: CALENDLY_URL, external: true }] },
  ...
  return (
    <footer style={{ background: "linear-gradient(180deg, #1E2270 0%, #181D5C 100%)", padding: "96px 48px 44px" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div className="footer-grid" style={{ display: "grid", gridTemplateColumns: "1.6fr repeat(3,1fr)", gap: 44, marginBottom: 80 }}>
          <div>
          ...
          {cols.map(c => <div key={c.title}><div style={{ fontFamily: F.mono, fontSize: 9.5, letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(255,255,255,0.45)", marginBottom: 22 }}>{c.title}</div>
            {c.items.map(i => i.href
                ? <a key={i.l} href={i.href} target={i.external ? "_blank" : undefined} rel={i.external ? "noopener noreferrer" : undefined} style={{ fontFamily: F.sans, fontSize: 13, color: "rgba(255,255,255,0.55)", padding: "5px 0", display: "block", textDecoration: "none", transition: "all 0.3s", fontWeight: 350 }} onMouseEnter={e => e.target.style.color = "rgba(255,255,255,0.9)"} onMouseLeave={e => e.target.style.color = "rgba(255,255,255,0.55)"}>{i.l}</a>
